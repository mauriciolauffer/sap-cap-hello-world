...
locals {
  subaccount_domain = "rag${random_string.gen.result}"
...
### Subaccount
##########
resource "btp_subaccount" "rag_workshop" {
...
resource "btp_subaccount_entitlement" "cloud_foundry_quota" {
  subaccount_id = btp_subaccount.rag_workshop.id
